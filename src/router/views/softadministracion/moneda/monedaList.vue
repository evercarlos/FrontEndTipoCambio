...
export default ({
    ...
    setup(props, context) {
      ...
      const title = ref('Lista las Monedas')
      const items = ref([
        {
          text: "Administración",
          href: "/",
      ...
      const fields = ref([
        { key: 'idMoneda', sortable: true, thClass: 'd-none', tdClass: 'd-none' },
      ...
      const listRecords = async () => {
        isBusy.value = true
        await store.dispatch('softadministracion/MONEDA_FIND_ALL', {
          limit: perPage.value,
          query: query.value,
          page: page.value,
      ...
      const openForm =type => {
        formType.value = type
        titleModal.value = 'Nueva Moneda'
        modalShow.value = true
      }
      const getEdit = data => {
        formType.value = 'edit'
        titleModal.value = 'Editar Moneda'
      ...
      const getDelete = async data => {
        ...
        if (result.value) {
         store
          .dispatch('softadministracion/ONEDA_DELETE', {
              id: data.item.idMoneda,
            })
            .then(() => {
